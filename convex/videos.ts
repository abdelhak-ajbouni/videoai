  ...
  handler: async (ctx, args) => {
    ...
    const analytics = {
      ...
      totalDownloads: videos.reduce(
        (sum, v) => sum + (v.downloadCount || 0),
        0
      ),
      totalCreditsUsed: videos.reduce((sum, v) => sum + v.creditsCost, 0),
      totalFileSize: videos.reduce((sum, v) => sum + (v.fileSize || 0), 0),
      averageProcessingTime: videos
        .filter((v) => v.processingDuration)
        .reduce(
          (sum, v, _, arr) => sum + v.processingDuration! / arr.length,
          0
        ),
      qualityBreakdown: {
        standard: videos.filter((v) => v.quality === "standard").length,
        high: videos.filter((v) => v.quality === "high").length,
        ultra: videos.filter((v) => v.quality === "ultra").length,
      },
      statusBreakdown: {
        ...
        canceled: videos.filter((v) => v.status === "canceled").length,
      },
    };
